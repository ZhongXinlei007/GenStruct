...
def file_f1(file_name, test_field):
    """通用评测：
    - 若 test_field 是数值/可转为数值（如 final_pred_num），则直接与 ans_id 比较。
    - 否则按旧逻辑从文本里解析预测实体名。
    """
    all_num = 0
    hit_num = 0
    ...
    with open(file_name) as input_f:
        for line in input_f:
            ...
            if test_field not in line:
                error_num += 1
                continue

            pred_val = line[test_field]
            ...
            if isinstance(pred_val, (int, float)) or (isinstance(pred_val, str) and pred_val.strip().lstrip('-').isdigit()):
                try:
                    ...
                    error_num += 1
                    continue
                if pred_id == line.get('ans_id'):
                    hit_num += 1
                continue

            # 情况2：文本（LLM 输出），回退到旧解析
            if not isinstance(pred_val, str) or len(pred_val) == 0:
                error_num += 1
                continue

            map_dict = {'none':4096}
            for cand in line.get('candidates', []):
                if 'wiki_id' in cand and 'name' in cand:
                    try:
                        map_dict[str(cand['name']).lower()] = int(cand['wiki_id'])
                    except Exception:
                        pass

            pred_entity = result_decode(pred_val, map_dict)
            pred_num = map_dict.get(pred_entity, -1)

            if pred_num == line.get('ans_id'):
                hit_num += 1
            
    acc = (hit_num / all_num) if all_num > 0 else 0.0
    print(all_num, hit_num, acc ,error_num)
